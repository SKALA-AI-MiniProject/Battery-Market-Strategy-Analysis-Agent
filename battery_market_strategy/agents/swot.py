...
class SWOTAgent(BaseAgent):
    ...
    def run(self, state: GraphState) -> dict:
        ...
        system_prompt = (
            ...
            "Be internally consistent with the supplied company analysis. Do not create a weakness that directly contradicts a stated strength unless you explain the mechanism or trade-off. "
            "Strengths and weaknesses must be internal factors. Opportunities and threats must be external conditions."
        )
        user_prompt = (
            f"회사: {self.company}\n\n"
            f"시장 분석:\n{market['market_view']}\n"
            ...
            "특허 수, 생산능력 규모, 투자 계획 자체를 자동으로 강점으로 두지 말고, 그것이 실제 실행력·원가·현지화·제품력으로 연결될 때만 강점으로 인정하라.\n"
            "반대로 수치가 크다는 이유만으로 약점으로 뒤집는 자기모순을 만들지 마라.\n"
            "각 항목은 짧고 압축적으로 쓰되, 같은 내용의 재서술은 피하라."
        )
