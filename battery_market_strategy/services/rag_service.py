...
class AgenticRAGService:
    ...
    def _reflect(self, company: str, refined_query: str, chunks: list[dict]) -> RetrievalReflectionOutput:
        system_prompt = (
            ...
            "Do not approve an evidence set too quickly if it supports only strengths and expansion claims but lacks any indication of constraints, risks, delays, or execution limits. "
            "However, do not request exact customer lists, competitor weaknesses, precise market-share tables, recycling profitability, or service-expansion specifics unless the current evidence is otherwise too thin to support a basic company profile. "
            "Do not request exact financial burdens, detailed profitability metrics, full product lineups, or explicit peer-comparison language unless those details are clearly necessary to establish even a minimal company profile. "
        ...
        user_prompt = (
            ...
            "If the evidence set sounds one-sided or promotional, look for whether the current chunks already contain constraints, limitations, risk language, or execution caveats before approving it. "
            "If optional external counter-evidence snippets are empty or unavailable, treat that as neutral and do not use the absence itself as a blocker. "
            "If the current evidence already supports a balanced high-level summary, do not ask for extra retrieval only to obtain more exact risk metrics, market-share tables, customer lists, or competitor-comparison passages. "
